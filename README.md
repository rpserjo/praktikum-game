...
6. `Prod` Запустить локально сервер командой node dist/index.js (сервер запустится по адресу http://localhost:3001)</br>
...
* запустить локально сервер командой `yarn dev` или `yarn cross-env NODE_ENV=development node dist/index.js` (сервер запустится по адресу http://localhost:3001)
